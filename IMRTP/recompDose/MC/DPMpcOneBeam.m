function IMCalc = DPMpcOneBeam(leak, spectrum_File, planC_File, nhist, OutputError, whichBeam, PBMaxWidth, gradsense, MCsolver, saveIM, sourceModel, doseToWater, fillWater, useWedge, inputPB, inputIM, Softening, UseFlatFilter, MLC, TongueGroove, batch, interactiveMode, LS, MU)
% written by KZ, JC
...
elseif interactiveMode && ~exist('LS','var')
    ...
    if (MCsolver == 1)
        ...
        [IMCalc doseV indV numberParticles] = generateDPMdose_aavc(IMCalc,spectrum_File,nhist, OutputError,planC, indexBeam, w_field, saveIM, sourceModel, fillWater, useWedge, Softening,UseFlatFilter);
...
elseif interactiveMode && exist('LS','var')
    ...
    [inflMap, xV, yV, colDividerXCoord, rowDividerYCoord, rowLeafPositions] = ...
        getLSInfluenceMapFactorTG(precisionFluenceMap, LS,leak, indexBeam, MLC, TongueGroove); %leak,bs.BeamNumber=1
    ...
    if (MCsolver == 1)
        ...
        [IMCalc doseV indV numberParticles] = generateDPMdose_aavc(IMCalc,spectrum_File,nhist, OutputError,planC, indexBeam, w_field, saveIM, sourceModel, fillWater, useWedge, Softening, UseFlatFilter, MU);
