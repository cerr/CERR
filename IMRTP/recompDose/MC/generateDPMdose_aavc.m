function [IM doseV_SUM indV sampleParticles] = generateDPMdose_aavc (IM, Energy, nhist, OutputError, planC, indBeam, w_field, saveIM, sourceModel, fillWater, useWedge, Softening, UseFlatFilter, K)
%"generateMCInfluence"
...
scanNum = 1;
scan = getUniformizedCTScan(0, scanNum);
...
[xV, yV, zV] = getUniformizedXYZVals_plnChk(planC);
...
% Feb 29, 2008
% To treat everything above the maximum density (which is bone now) as
% bone.
CTscan.density(find(CTscan.density > materialMap(end).defaultDensity)) ...
    = materialMap(end).defaultDensity;
CTscan.material(find(CTscan.material == 0)) = length(materialMap);
...
if (sourceModel == 1)  % Use source model, version 1.
    ...
    CosHorn = CosHorn';
    cosOffAxisAngle = IM.beams(beamIndex).isodistance ./ ...
        (sqrt((IM.beams(beamIndex).RTOGPBVectorsM_MC(:,1).^2 +IM.beams(beamIndex).RTOGPBVectorsM_MC(:,2).^2+ IM.beams(beamIndex).RTOGPBVectorsM_MC(:,3).^2)));
...
%JC Nov 16, 2006, Use the most up-to-date DPM.
% Add more input to DPMIN.
DPMIN.UseFlatFilter = IM.params.DPM.UseFlatFilter;          % If it's 1, use FlatFilter.
...
attenuation = ones(size(IM.beams(beamIndex).xPBPosV));
...
IM.beams(beamIndex).beamlets = [];
%for i= 1:length(IM.beams(beamIndex).beamletDelta_x), %APA
for i= 1:length(IM.beams(beamIndex).xPBPosV), %APA
    ...
    dx=IM.beams(beamIndex).beamletDelta_x(i);  %APA was (i)
    dy=IM.beams(beamIndex).beamletDelta_y(i);  %APA was (i)
    ...
    coll3V(i,:) = scan2Collimator(pb(i,:), th, cA, clA, IC,IM.beams(beamIndex).isodistance);
    ...
    tic; [dosePrimary sampleParticles] = dpm(DPMIN, CTscan); toc
    ...
    if (sourceModel ==1)    % Use source model, version 1.
        ...
        tic; [doseFF sampleParticles]  = dpm(DPMIN, CTscan); toc
        ...
        if (SourceModelParams(8) > 0.005) %If the relative fluence of electron contamination is less than 0.5%, then call DPM for electron.
            ...
            tic; [doseElec  sampleParticles]= dpm(DPMIN, CTscan); 
        else
        ...
        % Change the dose from dose / particle to dose in Gy/MU
        doseV = doseV*K;
    ...
    if (saveIM == 1)
        ...
        if length(indV)==length(doseV)
            ...
            errorBeamlets = [errorBeamlets beamletCount];
        end

        %-----------Insert dose data into correct beamlet indices---------------------%
        for j = 1 : length(structROIV)
            beamlet = createIMBeamlet(doseM(scanIndV{j}), scanIndV{j}, beamIndex, 0);
            beamlet.structureName = planC{planC{end}.structures}(structROIV(j)).structureName;
            beamlet.sampleRate = sampleRateV(j);
            beamlet.strUID = planC{planC{end}.structures}(structROIV(j)).strUID;
            if isempty(IM.beams(beamIndex).beamlets)
                IM.beams(beamIndex).beamlets = beamlet;
            else
                IM.beams(beamIndex).beamlets(j,beamletCount) = beamlet;
            end
        end
