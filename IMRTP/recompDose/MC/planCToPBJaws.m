...
planCToPBJaws(planC, beamNumber, bResx, bResy);
...
% 3 entries, then this is a square field defined by collimator
  s=planC{indexS.beamGeometry}(beamNumber).file{4};
  ind=max(strfind(s, '"'));
  ...
  s=planC{indexS.beamGeometry}(beamNumber).file{5};
  ind=max(strfind(s, '"'));
...
if(length(planC{indexS.beamGeometry}(beamNumber).file)<6), 
