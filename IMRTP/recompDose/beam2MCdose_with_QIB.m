function [IMCalc, w_field] = beam2MCdose_with_QIB(leak, spectrum_File, planC_File, nhist, OutputError, whichBeam, PBMaxWidth, gradsense, MCsolver, saveIM, sourceModel, doseToWater, fillWater, useWedge, inputPB, inputIM, Softening, UseFlatFilter, MLC, TongueGroove, batch, priOptMode, LS_flag, K, scatterThreshold, structNumsV, sampleRateV)
% written by KZ, JC
...
% JC Feb 27 06
% Call in DPMInfluence.m; use sum(10*clock) as the random seeds input to
% DPM.
...
% Now, only work for DPM, planC{indexS.beams} not empty.

if ischar(inputIM)
    inputIM = str2num(inputIM)           %Flag, inputIM == 1, ask user to input .mat file storing IM info.
    % That file can be generated using
    % Thuse the planC{indexS.beams} and planC{6} will not
    % be used.
...
if ~isempty(planC{indexS.beams}) && ((isfield(planC{indexS.beams}(1).FractionGroupSequence.Item_1,'NumberofBeams') && planC{indexS.beams}(1).FractionGroupSequence.Item_1.NumberofBeams) || (isfield(planC{indexS.beams}(1).FractionGroupSequence.Item_1,'NumberOfBeams') && planC{indexS.beams}(1).FractionGroupSequence.Item_1.NumberOfBeams))
    % When there are multiple beam definition in planC, use the first BEAM
    % data as the default.

    %for indexBeam = 1 : planC{indexS.beams}(1).FractionGroupSequence.Item_1.NumberOfBeams
    for indexBeam = whichBeam
        bf = planC{indexS.beams}(1).FractionGroupSequence.Item_1.ReferencedBeamSequence.(['Item_' num2str(indexBeam)]);
        %if (bf.BeamDose ~= 0 & indexBeam ~= 2)
        if (bf.BeamDose ~= 0)
            bs = planC{indexS.beams}(1).BeamSequence.(['Item_' num2str(indexBeam)]);
            ...
            if ~isfield(planC{indexS.beams}(1).PatientSetupSequence,(['Item_' num2str(indexBeam)]))
                position = {planC{indexS.beams}(1).PatientSetupSequence.(['Item_' num2str(1)]).PatientPosition};
            else
                position = {planC{indexS.beams}(1).PatientSetupSequence.(['Item_' num2str(indexBeam)]).PatientPosition};
            end
            ...
            if abs(IMBase.beams(1).beamEnergy-6) < abs(IMBase.beams(1).beamEnergy-18)
                IMBase.beams(1).beamEnergy = 6;
            else
                IMBase.beams(1).beamEnergy = 18;
            end
            ...
            if MCsolver ~= 3 %Calculate dose within Skin structure
                
                
                structNum = find(strcmpi({planC{indexS.structures}.structureName}, 'Skin'));

                if isempty(structNum)
                    structNum = find(strcmpi({planC{indexS.structures}.structureName}, 'Body'));
                end

                if isempty(structNum)
                    structNum = find(strcmpi({planC{indexS.structures}.structureName}, 'External'));
                end
                
                if isempty(structNum)
                    error('Skin/Body/External structure must be defined to use DPM.');
                else
                    structNum = structNum(1);
                end
                
                IMBase.goals.structNum = structNum;
                IMBase.goals.structName = planC{indexS.structures}(structNum).structureName;
                IMBase.goals.strUID = planC{indexS.structures}(structNum).strUID;
                IMBase.params.xyDownsampleIndex = 2;
                IMBase.goals.isTarget(1) = 'y';
                IMBase.goals.xySampleRate = 2;
                IMBase.params.numCTSamplePts = 300;

            else %Calculate dose within Skin structure
                
                %Assume scalar beamlet_delta_x and beamlet_delta_y
                beamlet_delta_x = beamlet_delta_x(1);
                beamlet_delta_y = beamlet_delta_y(1);

                for i=1:length(structNumsV)
                    structNum = structNumsV(i);
                    IMBase.goals(i).structNum = structNum;
                    IMBase.goals(i).structName = planC{indexS.structures}(structNum).structureName;
                    IMBase.goals(i).strUID = planC{indexS.structures}(structNum).strUID;
                    IMBase.goals(i).isTarget = 'y';
                    IMBase.goals(i).xySampleRate = sampleRateV(i);
                    IMBase.goals(i).PBMargin = 0.5;
                end                
            end
...
%                 getPBRays_planChk(xPosV, yPosV, beamlet_delta_x, beamlet_delta_y, sourceS);
            ...
            %edgeS = getTargetSurfacePoints(IMBase,planC);
            [CTTraceS, RTOGPBVectorsM, RTOGPBVectorsM_MC, PBMaskM, rowPBV, colPBV, xPBPosV, yPBPosV] = ...
                getPBRayData_cerr3(sourceS, IMBase.params.numCTSamplePts, IMBase.params.xyDownsampleIndex, planC, xPosV, yPosV, beamlet_delta_x, beamlet_delta_y, gA);
            ...
            IMCalc.beams(1).CTTraceS          = CTTraceS;
            ...
            if (MCsolver == 1)
                ...
                currDir = cd;
                dpmDir = which('dpm.dll');
                dpmDir = fileparts(dpmDir);
                cd(dpmDir)                
            ...
            elseif (MCsolver == 3)
                %QIB params
                IMCalc.beams(1).sigma_100 = 0.4;
                IMCalc.beams(1).beamDescription = 'Eclipse Export';
                IMCalc.params.cutoffDistance = 5;
                IMCalc.params.algorithm = 'QIB';
                IMCalc.params.DoseTerm = 'gaussprimary+scatter'; %'nogauss+scatter';
                IMCalc = updateQIBInfluence(IMCalc, structNumsV, sampleRateV, ones(length(sampleRateV),1));
                for i=1:length(IMCalc.beamlets)
                    for j=1:length(structNumsV)
                        IMCalc.beamlets(j,i).beamNum = whichBeam;
                    end
                end                
                %dpmDir = which('dpm.dll');
                %dpmDir = fileparts(dpmDir);
            else
                error('INVALID input: MCsolver == 1, DPM; MCsolver == 2, VMC++; MCsolver == 3, QIB')
            end
...
if (MCsolver ~= 3)
    filename = fullfile(dpmDir,'storage',['dose3D_',num2str(indexBeam),'_', num2str(nhist), '_', num2str(batch),'.mat']);
...
%       end
% 	end
indTargetV      = strcmpi({IM.goals.isTarget}, 'y');
