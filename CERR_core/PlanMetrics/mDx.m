function [varargout] = mDx(planC, varargin)
%Calculate an mDx metric.
%Stand alone metric :       mDx(planC, structNum, doseNum, percent);
%Request m object   :       mDx(planC, 'getnewmetric');
%Evaluate m object  :       mDx(planC, m, 'evaluate');
...
if(nargin > 1)
    ...
    warning('Incorrect Usage, try: mDx(planC, structNum, doseNum, percent)');
    return;
end
...
switch upper(call)
    case 'GETNEWMETRIC'
        m.name = 'mDx';
        m.valueV = [];
        m.description = 'Returns the mean dose of the upper tail (the hottest x% of the structure)';
        m.functionName = @mDx;
        m.note = '';
    ...
    otherwise
        if(nargin > 3)
            ...
            varargout = {calc_mDx(doseBinsV, volsHistV, percent)};
        else
            error('Not enough parameters, try: mDx(planC, structNum, doseNum, percent)')
            return;
        end
end
