    ...
    switch(filterType)
        ...
        case 'LawsEnergy'
            ...
            for i = 1:length(fieldNameC)
                ...
                if isfield(paramS.padding,'method') &&...
                        strcmpi(paramS.padding.method,'expand')
                    valOrigPadV = [min(origPadV(1), minr),...
                        min(origPadV(1), origSizeV(1)-maxr),...
                        min(origPadV(2), minc), ...
                        min(origPadV(2), origSizeV(2)-maxc),...
                        min(origPadV(3), mins),...
                        min(origPadV(3), origSizeV(3)-maxs)];
                else
                    valOrigPadV = [origPadV(1),origPadV(1),origPadV(2),...
                        origPadV(2),origPadV(3),origPadV(3)];
                end

                calcMask3M = true(size(lawsTex3M));
                calcMask3M([1:valOrigPadV(1),...
                    origSizeV(1)-valOrigPadV(2)+1:origSizeV(1)],:,:) = false;
                calcMask3M(:,[1:valOrigPadV(3),...
                    origSizeV(2)-valOrigPadV(4)+1:origSizeV(2)],:) = false;
                calcMask3M(:,:,[1:valOrigPadV(5),...
                    origSizeV(3)-valOrigPadV(6)+1:origSizeV(3)]) = false;

                %Pad for mean filter
                [cropLawsTex3M,outMask3M,extentsV] = padScan(lawsTex3M,...
                    calcMask3M,padMethod,padSizV,cropFlag);
                filtRespSizeV = size(lawsTex3M);
                ...
                %Remove padding
                if strcmpi(padMethod,'expand')
                    valEnergyPadV = [min(padSizV(1), extentsV(1)),...
                        min(padSizV(1), filtRespSizeV(1)-extentsV(2)),...
                        min(padSizV(2), extentsV(3)), ...
                        min(padSizV(2), filtRespSizeV(2)-extentsV(4)),...
                        min(padSizV(3), extentsV(5)),...
                        min(padSizV(3), filtRespSizeV(3)-extentsV(6))];
                else
                    valEnergyPadV = [padSizV(1),padSizV(1),padSizV(2),...
                        padSizV(2),padSizV(3),padSizV(3)];
                end
                lawsEnergy3M = lawsEnergy3M(valEnergyPadV(1)+1:...
                    padResponseSizeV(1)-valEnergyPadV(2),...
                    valEnergyPadV(3)+1:padResponseSizeV(2)-valEnergyPadV(4),...
                    valEnergyPadV(5)+1:padResponseSizeV(3)-valEnergyPadV(6));

                %Reapply original padding
                %lawsEnergyPad3M = nan(origSizeV);
                lawsEnergyPad3M = lawsTex3M;
                lawsEnergyPad3M(valOrigPadV(1)+1:end-valOrigPadV(2),...
                valOrigPadV(3)+1:end-valOrigPadV(4),valOrigPadV(5)+1:...
                end-valOrigPadV(6)) = lawsEnergy3M;

                %cropSizeV = size(lawsEnergy3M);
                %lawsEnergyPad3M([1:valOrigPadV(1),end-valOrigPadV(2)+1:end],:,:) = ...
                %lawsTex3M([1:valOrigPadV(1),end-valOrigPadV(2)+1:end],:,:);
                %lawsEnergyPad3M(:,[1:valOrigPadV(3),end-valOrigPadV(4)+1:end],:) = ...
                %lawsTex3M(:,[1:valOrigPadV(3),end-valOrigPadV(4)+1:end],:);
                %lawsEnergyPad3M(:,:,[1:valOrigPadV(5),end-valOrigPadV(6)+1:end]) = ...
                %lawsTex3M(:,:,[1:valOrigPadV(5),end-valOrigPadV(6)+1:end]);
