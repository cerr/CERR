function slcWavScanM = wavDecom3D(scan3M,dirString,wavType)
...
if mod(size(scanRowM,1),2)==1
    scanRowM = padarray(scanRowM,[1,0],'replicate','pre');
    padFlag=1;
end
...
for n = 1:size(scanRowM,2)
    ...
    wavImgM = modwt(scanRowM(:,n),wavType,1);
    if dirString(1) == 'L'
...
if mod(size(scanColM,1),2)==1
    scanColM = padarray(scanColM,[1,0],'replicate','pre');
    padFlag=1;
end
...
for n = 1:size(scanColM,2)
    ...
    wavImgM = modwt(scanColM(:,n),wavType,1);
    if dirString(1) == 'L'
...
if length(dirString) > 2
    ...
    scanSlcM = permute(colWavScanM,[3 1 2]);
    scanSlcM = reshape(scanSlcM,[siz(3),siz(1)*siz(2)]);
    padFlag = 0;
    if mod(size(scanSlcM,1),2)==1
        scanSlcM = padarray(scanSlcM,[1,0],'replicate','pre');
        padFlag=1;
    end
    ...
    for n = 1:size(scanSlcM,2)
        ...
        wavImgM = modwt(scanSlcM(:,n),wavType,1);
        if dirString(1) == 'L'
