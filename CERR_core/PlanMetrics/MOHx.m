function [varargout] = MOHx(planC, varargin)
%Calculate a MOHx metric.
%Stand alone metric :       MOHx(planC, structNum, doseNum, percent);
%Request m object   :       MOHx(planC, 'getnewmetric');
%Evaluate m object  :       MOHx(planC, m, 'evaluate');
%LM:  4 Jan 06, JOD: changed interface.
%LM: 26 Jun 06, VHC: turned from Dx to mDx (only does upper tail currently)
%LM: 20 Nov 06, VHC: turned from mDx to MOHx
%
...
if(nargin > 1)
    ...
    warning('Incorrect Usage, try: MOHx(planC, structNum, doseNum, percent)');
    return;
end
...
switch upper(call)
    case 'GETNEWMETRIC'
        m.name = 'MOHx';
        m.valueV = [];
        m.description = 'Returns the mean dose of the hottest x% of the structure (the upper tail)';
        m.functionName = @MOHx;
        m.note = '';
    ...
    otherwise
        if(nargin > 3)
            ...
            varargout = {calc_MOHx(doseBinsV, volsHistV, percent)};
        else
            error('Not enough parameters, try: MOHx(planC, structNum, doseNum, percent)')
            return;
        end
end
