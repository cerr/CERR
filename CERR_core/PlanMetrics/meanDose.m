function [varargout] = meanDose(planC, varargin)
%Calculate a meanDose metric.
%Stand alone metric :       meanDose(planC, structNum, doseNum, 'Percent' or 'Absolute');
%Request m object   :       meanDose(planC, 'getnewmetric');
%Evaluate m object  :       meanDose(planC, m, 'evaluate');
...
if(nargin > 1)
    ...
    warning('Incorrect Usage, try: meanDose(planC, structNum, doseNum, ''Percent'' or ''Absolute'');');
    return;
end
...
switch upper(call)
    case 'GETNEWMETRIC'
        m.name = 'meanDose';
        m.valueV = [];
        m.description = 'Returns the mean dose in specified structure';
        m.functionName = @meanDose;
        m.note = '';
    ...
    case 'EVALUATE'
		...
		for i=1:length(m.doseSets)
            ...
            ans = calc_meanDose(doseBinsV, volsHistV, m.params(2).value);
    ...
    otherwise  %No specific call has been made, assume user just wants raw metric.
        if(nargin > 3)
            ...
            varargout = {calc_meanDose(doseBinsV, volsHistV, volumeType)};
        else
            error('Not enough parameters, try: meanDose(planC, structNum, doseNum, ''Percent'' or ''Absolute'');')
            return;
        end
    end
