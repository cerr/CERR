function ans = calc_MOHx(doseBinsV, volsHistV, percent, volType)
% Returns the mean dose of the upper tail (the hottest x% of the structure)
...
%  VHC modified 11/20/06, changed from calc_mDx to calc_MOHx
%
%  Usage: calc_MOHx(doseBinsV, volsHistV, percent)
%
...
if ~exist('volType','var')
    ...
    if ~volType
        percent = percent/sum(volsHistV)*100;
    end
end
