function [planC,hiSuvCtStrV,hiSuvPetStrV] = ...
    preprocessScansForHypoxiaSign(structNumV,badSlicesStr,planC)
% function [planC,hiSuvCtStrV,hiSuvPetStrV] = ...
%     preprocessScansForHypoxiaSign(structNumV,badSlicesStr,planC)
%
% REQUIREMENTS: 
...
% Planning CT scan at index 3 within planC{indexS.scan}
% Additionally, the GTV structures must be defined on the plannnig CT i.e.
% scan number 3. 
%
% INPUTS: 
% % Structure numbers defined on scan number 3
% structNumV = [3,4,5];
% % Bad (dental artifact) slices to ignore from planning CT
% badSlicesStrC = '60-62'; % 'None' for no artifact slices
%
% OUTPUTS:
% planC: planC object with the additional structures on FDG-CT and FDG-PET.
...
[~,zminBad,zMaxBad,zOffset] = getBadSlicesZs(planCT_nonDef_scanNum, badSlicesStr, planC, indexS);
...
for iStr = 1:length(structNumV)
    ...
    % Copy structure (incl. bad slices) from planning to FDG PET
    ...
    planC = copyStrToScanAndInterp(numStructs, fdgCT_scanNum, noBad_maskStruc, planC);
