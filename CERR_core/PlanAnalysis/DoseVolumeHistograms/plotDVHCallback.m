function plotDVHCallback(command, varargin)
%"plotDVHCallback"
...
switch upper(command)
    ...
    %Request to expand the DVH view.
        ...
        set(gcbo, 'label', 'Simple Options', 'callback',['plotDVHCallback(''SIMPLEVIEW'')']);
        
        %Actually perform the initialization.
        if ~isfield(ud, 'handles')
            plotDVHCallback('INITEXPANDED');
            ...
            try
                ...
                if n > 0
                    ...
                    set(hAxis, 'buttonDownFcn', 'plotDVHCallback(''CLICKINPLOT'')');                    
                end
            end
        end
        for i=1:length(ud.plots)
            set(ud.plots(i).hLine, 'hittest', 'on', 'buttondownfcn',['plotDVHCallback(''DVHCLICKED'', ' num2str(i) ')']);
        end
    ...
    %Return to the simple view by contracting the margin.    
        ...
        set(gcbo, 'label', 'Expanded Options', 'callback',['plotDVHCallback(''EXPANDEDVIEW'')']);        
    ...
    %If this is the first time the expanded view is requested, draw all the uicontrols.    
        ...
        ud.handles.strList = uicontrol(hFig, 'style', 'popupmenu', 'units', units, 'position', [frame.X+90 frame.Y+frame.H-25 80 15], 'string', strNames, 'horizontalAlignment', 'left', 'callback', 'plotDVHCallback(''SELECTDVH'');');
        ...
        ud.handles.meanTxt = uicontrol(hFig, 'style', 'text', 'units', units, 'position', [frame.X+20 frame.Y+frame.H-125 80 15], 'string', 'Mean Dose:', 'horizontalAlignment', 'left');
        ...
        ud.handles.maxTxt = uicontrol(hFig, 'style', 'text', 'units', units, 'position', [frame.X+20 frame.Y+frame.H-175 80 15], 'string', 'Max Dose:', 'horizontalAlignment', 'left');
        ...
        ud.handles.minTxt = uicontrol(hFig, 'style', 'text', 'units', units, 'position', [frame.X+20 frame.Y+frame.H-200 80 15], 'string', 'Min Dose:', 'horizontalAlignment', 'left');
        ...
        uicontrol(hFig, 'style', 'text', 'units', units, 'position', [frame.X+10 frame.Y+frame.H-275 80 15], 'string', 'Dose', 'horizontalAlignment', 'center');
        ...
        ud.handles.scanDoseVal = uicontrol(hFig, 'style', 'edit', 'units', units, 'position', [frame.X+10 frame.Y+frame.H-300 80 20], 'string', '.', 'horizontalAlignment', 'center', 'callback', 'plotDVHCallback(''DOSEVAL'')');    
        ...
        ud.handles.legendChk = uicontrol(hFig, 'style', 'checkbox', 'units', units, 'position', [frame.X+10 frame.Y+frame.H-350 150 20], 'string', 'Show Legend', 'horizontalAlignment', 'center', 'callback', 'plotDVHCallback(''LEGENDCALL'')');
        
        %control to export DVH to excel
        ud.handles.exportDVH = uicontrol(hFig, 'style', 'push', 'units', units, 'position', [frame.X+10 frame.Y+frame.H-380 100 20], 'string', 'Export DVH', 'horizontalAlignment', 'center', 'callback', 'plotDVHCallback(''EXPORTDVH'')');
    ...
    case 'SELECTDVH'
        %A DVH has been selected from the list, populate fields.
        ...
        if val > 0
            ...
            %If we are dealing with a DVH, rename fields and use different
            %metrics calculations from DSH.                                     
            elseif strcmpi(ud.plots(val).type, 'DVH')
                ...
                minD = doseBinsV(ind);
                set(ud.handles.statTxt, 'string', 'DVH Stats:')
                ...
                error('Not a valid plot type for DVH callback.')
            end
        else
    ...
    %A DVH is selected and the plot has been clicked, update guide line/fields.    
        ...
        hFig = get(hAxis, 'parent');
        ud = get(hFig, 'userdata');
        set(hFig, 'WindowButtonUpFcn', 'plotDVHCallback(''UNCLICKINPLOT'')');
        set(hFig, 'WindowButtonMotionFcn', 'plotDVHCallback(''PLOTMOTION'')');
        currentPlot = get(ud.handles.strList, 'value') - 1;
    ...
    case 'LEGENDCALL'
        ...
        for i = 1:numLines
            ...
            txt = ['Struct = ',ud.plots(i).struct,', Dose Fract = ',ud.plots(i).doseName];
    ...
    case 'EXPORTDVH'
        hFig = get(gcbo, 'parent');        
        ud = get(hFig, 'userdata');
        currentDVH = get(ud.handles.strList, 'value') - 1;
        if currentDVH == 0
            warndlg('Please select DVH to be exported.','DVH not selected','modal')
            return;
        end
        ...
        [fname, pname] = uiputfile('*.xls','Save the DVH data as:');
        if isnumeric(fname)
            return;
        end
        try
