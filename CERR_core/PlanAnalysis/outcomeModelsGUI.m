...
switch upper(command)
    ...
    case 'REFRESH'
        ...
        inputH(2) = uicontrol(hFig,'tag','modelTitle','units','pixels',...
            'Position',[2*shift posTop-.16*GUIHeight .26*GUIWidth 2*shift], 'String','','Style','text',...
    ...
    case 'PLOT_MODELS'
        ...
        for j = 1:numModels
            ...
            if isfield(modelC{j}, 'fractionCorrect') % fractionation correction
                numFractions = 5;
                abRatio      = 3;
                stdFractionSize = 2;
                oldDoseM = planC{indexS.dose}(doseNum).doseArray;
                correctedDoseM = planC{indexS.dose}(doseNum).doseArray;
                % EQD2
                correctedDoseM = correctedDoseM .* ...
                    (correctedDoseM/numFractions + abRatio) / (stdFractionSize + abRatio);
            ...
            % rescale dose back to the original 
            if isfield(modelC{j}, 'fractionCorrect') 
                planC{indexS.dose}(doseNum).doseArray = ...
                    oldDoseM;
            end
