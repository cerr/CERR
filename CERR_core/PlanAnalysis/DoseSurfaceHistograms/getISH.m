function [scanV, areaV, zV, planC] = getISH(structNum, scanNum, planC)
%"getDSH"
%   Returns DSH vectors for a specified structure and scanNum, where
...
if ~isempty(transMScan)  
  [xV, yV, zV] = applyTransM(inv(transMScan), xV, yV, zV);
end

areaV   = pointsM(:,4);
scanV   = getScanAt(scanNum, xV, yV, zV, planC);
scanV = scanV(:);
