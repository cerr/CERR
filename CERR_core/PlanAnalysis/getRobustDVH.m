function [dosesCurrentV, meanDoseV, stdDoseV, volsV, binWidth, doseBins, volsHist, volsHistStdV] = getRobustDVH(doseNum,structNum,numFractions,numTrials,XdispSys_Std,YdispSys_Std,ZdispSys_Std,XrotSys_Std,YrotSys_Std,ZrotSys_Std,XdispRnd_Std,YdispRnd_Std,ZdispRnd_Std,XrotRnd_Std,YrotRnd_Std,ZrotRnd_Std,planC,plotFlag)
%function [dosesV, meanDoseV, stdDoseV, volsV] = getRobustDVH(doseNum,structNum,numFractions,numTrials,XpdfStd,YpdfStd,ZpdfStd,planC)
%
...
if exist('plotFlag','var') && plotFlag == 1
    ...
    for iTrial=1:numTrials
        ...
        plot([0 doseBinsV], [1 cumVols2V/cumVolsV(end)],'k-');
    end
    xlabel('\bfDose')
    ylabel('\bfFractional volume')

    figure, hold on,

    %Mean-1xSigma
    [doseBinsMinV, volsHistMinV] = doseHist(max(0,meanDoseV - 1*stdDoseV), volsV, binWidth);
    ...
    plot([0 doseBinsMinV], [1 cumVolsMin2V/cumVolsMinV(end)],'r--');

    %Mean+1xSigma
    [doseBinsMaxV, volsHistMaxV] = doseHist(meanDoseV + 1*stdDoseV, volsV, binWidth);
    ...
    plot([0 doseBinsMaxV], [1 cumVolsMax2V/cumVolsMaxV(end)],'r--');

    patch([0 doseBinsMaxV fliplr(doseBinsMinV)], [1 cumVolsMax2V/cumVolsMaxV(end) fliplr(cumVolsMin2V)/cumVolsMinV(end)],'y','EdgeColor','None','faceAlpha',0.2);

    %Plot Current DVH
    cumVolsV  = cumsum(volsHistCurrentV);
    cumVols2V = cumVolsV(end) - cumVolsV;  %cumVolsV is the cumulative volume lt that corresponding dose
    plot([0 doseBinsCurrentV], [1 cumVols2V/cumVolsV(end)],'k-');
