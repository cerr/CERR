...
indexS = planC{end}; 
   ...
   StandardDeviationF = str2double(get(handles.viola_sdF, 'string'));
    StandardDeviationM = str2double(get(handles.viola_sdM, 'string'));
    SmootherVarianceF = str2double(get(handles.viola_svF, 'string'));
    SmootherVarianceM = str2double(get(handles.viola_svM, 'string'));
    SamplesNum = str2double(get(handles.viola_sn, 'string'));
    Minstep = str2double(get(handles.viola_minstep, 'string'));
    Maxstep = str2double(get(handles.viola_maxstep, 'string'));
    Iter_Num = str2double(get(handles.viola_in, 'string'));
    RegLevels = str2double(get(handles.viola_reglevels, 'string'));
    rscale = str2double(get(handles.viola_rscale, 'string'));
    tscale = str2double(get(handles.viola_tscale, 'string'));
    output = cell(1, 8);
...
%     end   
    ...
    tic;
    [im, TrackData, Offset] = Viola_Wells_MI2D(int16(FImg), originF, spacingF, ...
                                                int16(MImg), originM, spacingM, ...
                                                StandardDeviationF, StandardDeviationM, SmootherVarianceF, SmootherVarianceM, ...
                                                SamplesNum, Minstep, Maxstep, Iter_Num, RegLevels, rscale, tscale);
    ...
    output{8} = ['Metric Value = ' num2str(Offset(7))];
...
%update the transM;
    ...
    rot = eye(3); 
    rot(1) = cos(Offset(1)); rot(2,2) = rot(1); 
    rot(1,2) = -sin(Offset(1));
    rot(2,1) = -rot(1,2);
