function RegMeanSquareEuler(handles, metric)
%
...
indexS = planC{end}; 
    ...
    minstep = str2double(get(handles.Euler_minstep, 'string'));
    maxstep = str2double(get(handles.Euler_maxstep, 'string'));
    iternum = str2double(get(handles.Euler_IterNum, 'string'));
    
    RotationScale = str2double(get(handles.Euler_RS, 'string'));
    TranslationScale = str2double(get(handles.Euler_TS, 'string'));
    InitialAngle = str2double(get(handles.Euler_InitAngle, 'string'));
...
%call registration method
    try
        [im, Rotation, Offset, TrackData] = Euler3D(int16(FImg), originF, spacingF, ...
                                                int16(MImg), originM, spacingM, ...
                                                minstep, maxstep, iternum, ...
                                                RotationScale, TranslationScale, metricIndex, rotM, transV, initMode);
    catch
        [im, Rotation, Offset, TrackData] = Euler3D_64(int16(FImg), originF, spacingF, ...
                                                int16(MImg), originM, spacingM, ...
                                                minstep, maxstep, iternum, ...
                                                RotationScale, TranslationScale, metricIndex, rotM, transV, initMode);
    end
    ...
    output{10} = ['Offset X = ' num2str(Offset(12))];
    output{11} = ['Offset Y = ' num2str(Offset(13))];
    output{12} = ['Offset Z = ' num2str(Offset(14))];
        
    set(handles.OutputList, 'string', output);
    
%update the transM;
    Tv = [Offset(1) Offset(2) Offset(3)];
    Cv = [Offset(9) Offset(10) Offset(11)];
