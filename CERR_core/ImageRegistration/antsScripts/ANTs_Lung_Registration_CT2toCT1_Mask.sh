...
#SYNCONVERGENCE="[100x100x100x50x0,1e-6,10]"
#SYNCONVERGENCE="[60x30x10x0,1e-6,10]"
#SYNSHRINKFACTORS="8x4x2x1"
#SYNSHRINKFACTORS="8x6x4x2x1"
#SYNSMOOTHINGSIGMAS="2x1x0x0x0vox"
...
for(( i=0; i<${#FIXEDIMAGES[@]}; i++ ))
  do
    SYNMETRICS="$SYNMETRICS --metric MeanSquares[${FIXEDIMAGES[$i]},${MOVINGIMAGES[$i]},1,4,Regular,0.25]"
...
if [[ $TRANSFORMTYPE == 'so' ]] || [[ $TRANSFORMTYPE == 'br' ]];
  then
    SYNCONVERGENCE="[80x50x0,1e-6,10]"
...
if [[ $TRANSFORMTYPE == 's' ]] || [[ $TRANSFORMTYPE == 'sr' ]] || [[ $TRANSFORMTYPE == 'so' ]];
  then
    SYNSTAGE="--transform SyN[0.5,3,0] \
             $SYNSTAGE"
  fi
