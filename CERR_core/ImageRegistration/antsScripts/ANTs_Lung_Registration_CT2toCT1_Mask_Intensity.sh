...
AFFINESTAGE="--transform Affine[0.5] \
             --metric Mattes[${FIXEDIMAGES[0]},${MOVINGIMAGES[0]},1,32,Regular,0.25] \
...
for(( i=0; i<${#FIXEDIMAGES[@]}; i++ ))
  do
    #SYNMETRICS="$SYNMETRICS --metric MeanSquares[${FIXEDIMAGES[$i]},${MOVINGIMAGES[$i]},1,6,Regular,0.35]"
    #SYNMETRICS="$SYNMETRICS --metric CC[${FIXEDIMAGES[$i]},${MOVINGIMAGES[$i]},1,4,Regular,0.5]"
    SYNMETRICS="$SYNMETRICS --metric Mattes[${FIXEDIMAGES[$i]},${MOVINGIMAGES[$i]},1,${NUMBEROFBINS},Regular,0.5]"
  done
...
if [[ $TRANSFORMTYPE == 'so' ]] || [[ $TRANSFORMTYPE == 'br' ]];
  then
    SYNCONVERGENCE="[80x60x0,1e-6,10]"
...
patientNames=(pt1)
dataDir=../
outputDir="${dataDir}/Cases/"
inputDir="${dataDir}/Cases/"
input=${inputDir}/${patientNames}/
output="${outputDir}/${patientNames}/PlanSum/"
outputPrefixCT2CT1=${output}/${patientNames}_CT2toCT1_
outputPrefixCT2CT1CC=${output}/${patientNames}_CT2toCT1_CC_
#First plan eso contour expanded. It could be the original contour, rather than the expanded.
#${outputPrefixCT2CT1}Eso-label_exp.nrrd
#Second plan eso contour expanded 
#${outputPrefixCT2CT1CC}df_Eso-label_exp.nrrd

if [[ $TRANSFORMTYPE == 'b' ]] || [[ $TRANSFORMTYPE == 'br' ]] || [[ $TRANSFORMTYPE == 'bo' ]];
  then
    SYNSTAGE="--transform BSplineSyN[0.3,${SPLINEDISTANCE},0,3] \
    --masks [${outputPrefixCT2CT1}Eso-label_exp.nrrd,${outputPrefixCT2CT1CC}df_Eso-label_exp.nrrd] \
             $SYNSTAGE"
  fi

if [[ $TRANSFORMTYPE == 's' ]] || [[ $TRANSFORMTYPE == 'sr' ]] || [[ $TRANSFORMTYPE == 'so' ]];
  then
    SYNSTAGE="--transform SyN[0.8,3,0] \
             $SYNSTAGE"
  fi
...
#--winsorize-image-intensities [-1000,750]
