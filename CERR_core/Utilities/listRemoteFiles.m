function [remoteFiles, planC] = listRemoteFiles(planC, saveFlag)
%"listRemoteFiles"
...
%
%   If saveFlag is set to 1 or does not exist, the contents of the data
%   field for each remotely stored variable is written to that variable's
%   file.  The variable is then marked as unloaded and returned as part of
%   the modified planC.
%
%JRA 10/21/04
%
%Usage:
%   [remoteFiles, planC] = listRemoteFiles(planC, saveFlag)
%
    ...
    case 'struct'      
        for i=1:length(structure(:))
            ...
            for j=1:length(fieldNames)                           
                [structure(i).(fieldNames{j}), remoteFiles] = isWanted(structure(i).(fieldNames{j}), remoteFiles, saveFlag);                
            end
        end      
