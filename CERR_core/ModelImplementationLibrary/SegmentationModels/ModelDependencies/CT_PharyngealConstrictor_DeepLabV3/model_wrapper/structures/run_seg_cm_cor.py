...
# Date: Oct 10, 2019
# Description: Script to segment constrictor muscles from coronal CT images
# Architecture :  Deeplab v3+ with resnet backbone
# Usage: python run_seg_cm_cor.py [parameter_dictionary] [path to folder containing input 2D h5 files]
...
class Trainer(object):
    def __init__(self, argv):
        ...
        #modelPath = os.path.join(modelDir, 'finalModels/CM_Cor_model.pth.tar')
        modelPath = '/software/models/CM_Cor_model.pth.tar'
    ...
    def validation(self, inputH5Path):
        ...
        for filename in files:
            ...
            if count == 0:
                probMapAll = np.zeros((self.nclass, height, width, len(files)))

            prob_all = np.zeros((self.nclass, self.crop_size * self.crop_size))
    ...
    def transform_ts(self, sample):
        ...
        composed_transforms = transforms.Compose([
            ...
            tr.Normalize(mean=(0.3253, 0.3253, 0.3253), std=(0.1937, 0.1937, 0.1937)),
            tr.ToTensor()])
