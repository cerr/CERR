...
# Usage: python run_seg_larynx_cor.py [parameter_dictionary] [path to folder containing input 2D h5 files]
...
def main(argv, inputH5Path):
    ...
    probMapAll = trainer.validation(inputH5Path)
...
class Trainer(object):
    def __init__(self, argv):
        ...
        #modelPath = os.path.join(modelDir, 'finalModels/Larynx_Cor_model.pth.tar')
        modelPath = '/software/models/Larynx_Cor_model.pth.tar'
    ...
    def validation(self, inputH5Path):
        ...
        # Get h5 files in input dir
        files = find('*.h5', inputH5Path)
        files.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))

        print('Computing probability maps (cor view)...')
        count = 0
        # loop over the h5 files and return probability maps
    ...
    def transform_ts(self, sample):
        ...
        composed_transforms = transforms.Compose([
            ...
            tr.Normalize(mean=(0.3233, 0.3233, 0.3233), std=(0.1917, 0.1917, 0.1917)),
            tr.ToTensor()])
