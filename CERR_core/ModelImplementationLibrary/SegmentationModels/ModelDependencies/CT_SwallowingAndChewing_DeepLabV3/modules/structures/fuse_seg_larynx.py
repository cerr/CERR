...
def main(argv):
    ...
    t0 = time.time()
    probMapAx, fName = run_seg_larynx_ax.main(argv, inputH5PathAx)
    print(time.time() - t0)
    t1 = time.time()
    probMapSag = run_seg_larynx_sag.main(argv, inputH5PathSag)
    print(time.time() - t1)
    t2 = time.time()
    probMapCor = run_seg_larynx_cor.main(argv, inputH5PathCor)
    print(time.time() - t2)
